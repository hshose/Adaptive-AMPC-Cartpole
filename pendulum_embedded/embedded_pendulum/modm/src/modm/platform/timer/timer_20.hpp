...
#ifndef MODM_STM32_TIMER_20_HPP
#define MODM_STM32_TIMER_20_HPP
...
/**
 * @brief		Advanced Control Timer 20
 *
 * 
 *
 * Example:
 ...
 * MODM_ISR(TIM20_UP)
 * {
 *     Timer20::resetInterruptFlags(Timer20::...);
 *
 *     ...
 * }
 * @endcode
 ...
 */
class Timer20 : public AdvancedControlTimer
{
public:
	...
	connect()
	{
		using Connector = GpioConnector<Peripheral::Tim20, Signals...>;
		Connector::connect();
	}
	...
	{
		TIM20->CR2 =  (TIM20->CR2 & ~TIM_CR2_CCUS)
	...
	setPeriod(std::chrono::duration<Rep, Period> duration, bool autoApply = true)
	{
		...
		const uint32_t cycles = duration.count() * SystemClock::Timer20 * Period::num / Period::den;
	...
	{
		TIM20->EGR = static_cast<uint32_t>(ev);
	}
	...
	isCountingUp()
	{
		return (TIM20->CR1 & TIM_CR1_DIR) == 0;
	}
	...
	{
		constexpr auto channel = signalToChannel<Peripheral::Tim20, Signal>();
...
public:
	...
	{
		TIM20->DIER |= static_cast<uint32_t>(interrupt);
	}
	...
	{
		TIM20->DIER |= static_cast<uint32_t>(dmaRequests);
	}
	...
	getInterruptFlags()
	{
		return static_cast<InterruptFlag>(TIM20->SR);
	}
	...
	acknowledgeInterruptFlags(InterruptFlag flags)
	{
		...
		TIM20->SR = ~static_cast<uint32_t>(flags);
	}
};

}	// namespace modm::platform

#endif // MODM_STM32_TIMER_20_HPP